...
class FirebaseController extends GetxController with NotificationsService{
    ...
    try {
      await firebaseFirestore
        ...
        sendNotificationViaTopic(deviceId: device.deviceId,
          status: "Added", model: device.model, );
        isLoading(false);
  ...
  fetchDevicesByStatus(String status,) {
    return firebaseFirestore
        .collection('devices')
        .where('repairing', isEqualTo: status)
        .snapshots();
  }
  ...
  clearStorageData(){
    ...
    storage.read('model');
    storage.read('serial');
    storage.read('description');
    storage.read('images');
    storage.read('address');
    storage.read('issue');
    storage.read('security');
    storage.read('username');
    storage.read('accessories');
    storage.read('price');

  }
// Device device = Device(
//     deviceId: storage.read('device_id'),
//     brand: storage.read('brand'),
//     model: storage.read('model'),
//     serial: storage.read('serial'),
//     description: storage.read('description'),
//     images: storage.read('images'),
//     address: storage.read('address'),
//     phone: storage.read('phone'),
//     dateTime: Timestamp.now(),
//     repairingStatus: "Pending",
//     issue: storage.read('issue'),
//     ownerName: storage.read('owner'),
//     security: storage.read('security'),
//     uploadedBy: storage.read('username'),
//     storingStatus: 'In Store',
//     accessories: storage.read('accessories'),
//     repairingPrice: storage.read('price'),
//     completedNote: "",
//     cancelledNote: "",
//     inProgressNote: "",
//     returnedNote: ""
